...
# get rid of variables we don't want to use (rank_in_year, poster url, rating, release date, studio, title, rt_freshness)
...
library(e1071)
model_svm <- svm(formula = dataframetrainRanking ~ ., data = xtrain)
prediction_svm <- round(predict(model_svm, xtest))

results_svm = matrix(0, length(prediction_svm), 2)
for(i in 1:length(prediction_svm))
{
  results_svm[i,1] = prediction_svm[i]
  results_svm[i,2] = dataframetestRanking[i]
}

numcorrect_svm = 0
for(i in 1:length(prediction_svm))
{
  if(prediction_svm[i] == dataframetestRanking[i])
  {
    numcorrect_svm = numcorrect_svm + 1 
  }
}

accuracy_svm = numcorrect_svm/length(prediction_svm)
accuracy_svm

#randomforest
library(randomForest)
model_rf <- randomForest(x = xtrain, y = dataframetrainRanking)
prediction_rf <- round(predict(model_rf, xtest))

numcorrect_rf = 0
for(i in 1:length(prediction_rf))
{
  if(prediction_rf[i] == dataframetestRanking[i])
  {
    numcorrect_rf = numcorrect_rf + 1 
  }
}

accuracy_rf = numcorrect_rf/length(prediction_rf)
...
accuracy_knn = numcorrect_knn/length(prediction_knn)
